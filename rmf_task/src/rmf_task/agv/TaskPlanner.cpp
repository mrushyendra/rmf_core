...
private:
  ...
  Candidates(Map candidate_values, std::unordered_map<std::size_t, State>
    add_charging_task)
    : _value_map(std::move(candidate_values)),
    _add_charging_task(std::move(add_charging_task))
  {
    _update_map();
  }
  ...
  for (std::size_t i = 0; i < initial_states.size(); ++i)
  {
    ...
    if (finish.has_value())
    {
      initial_map.insert({
        ...
        Entry{i, finish.value().finish_state(),
...
struct Node
{
  ...
  friend std::ostream& operator<<(std::ostream& out, const Node& n)
  {
    ...
    for (const std::pair<const size_t, PendingTask>& unassigned : n.unassigned_tasks)
    {
      ...
      out << " ("
        << range.begin->second.state.finish_time().time_since_epoch().count()/1e9
        << ") ";
    }
    out << ">" << std::endl;
    return out;
  ...
  double compute_h(const Node& node, const rmf_traffic::Time time_now)
  {
    ...
    for (const auto& u : node.unassigned_tasks)
    {
      ...
      const rmf_traffic::Time earliest_deployment_time =
          u.second.candidates.best_finish_time()
          - u.second.request->invariant_duration();
      const double earliest_deployment_time_secs =
        rmf_traffic::time::to_seconds(earliest_deployment_time.time_since_epoch());

      const auto& range = u.second.candidates.best_candidates();
      for (auto it = range.begin; it != range.end; ++it)
      {
        const std::size_t candidate = it->second.candidate;
        if (earliest_deployment_time_secs < initial_queue_values[candidate])
          initial_queue_values[candidate] = earliest_deployment_time_secs;
      }
    }
      ...
      else
      {
        ...
        if (battery_estimate.has_value())
        {
          ...
          new_u.second.candidates.update_candidate(
            ...
            battery_estimate.value().wait_until()); // Impt to pass deployment time of charge task and not of the delivery task
